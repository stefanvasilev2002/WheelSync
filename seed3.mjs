// seed3.mjs — fix vehicle mileages then insert all logs
// Assignments already done; this runs mileage + fuel + service + defects + reminders
...
async function main() {
  ...
  // ── Reset vehicle mileages below first log entry ────────────────────────
  // Each vehicle needs currentMileage <= first log's startMileage
  console.log('Resetting vehicle mileages...');
  const vehicles = [
    // id, make, model, year, vin, licensePlate, color, engineType, fuelType, newMileage, companyId
    [1, 'Mercedes-Benz', 'Sprinter 316 CDI', 2021, '1HGBH41JXMN109186', 'SK-1234-AB', 'Bela',     '2.2 CDI',    'DIESEL', 81000, 1],
    [2, 'Volkswagen',    'Crafter 35 TDI',   2020, '2T1BURHE0JC068800', 'SK-5678-CD', 'Sina',     '2.0 TDI',    'DIESEL', 119000, 1],
    [3, 'Ford',          'Transit Custom',   2022, '3VWFE21C04M000001', 'SK-9012-EF', 'Srebrena', '2.0 EcoBlue','DIESEL', 43000, 1],
    [4, 'Toyota',        'Hilux 2.4 D-4D',  2023, '4T1BF3EK8AU567890', 'SK-2468-KL', 'Crna',     '2.4 D-4D',   'DIESEL', 11000, 1],
    [5, 'Iveco',         'Daily 35S14',      2019, '5YFBURHE0HP709746', 'SK-3456-GH', 'Bela',     '2.3 HPI',    'DIESEL', 193000, 2],
    [6, 'Renault',       'Master dCi 150',   2021, '6G1ZD5ST7KL100001', 'SK-7890-IJ', 'Zolta',    '2.3 dCi',    'DIESEL', 59000, 2],
  ];
  for (const [id, make, model, year, vin, licensePlate, color, engineType, fuelType, currentMileage, companyId] of vehicles) {
    await put(`/vehicles/${id}`, { make, model, year, vin, licensePlate, color, engineType, fuelType, currentMileage, companyId }, adm);
  }
  console.log('  Vehicle mileages reset');

  // ── Mileage logs ─────────────────────────────────────────────────────────
  console.log('Mileage logs...');
  const mileage = [
    [1, '2026-01-05', 82000,  82350,  'Ruta Skopje-Bitola',                   dt1],
    [1, '2026-01-15', 82350,  82890,  'Ruta Skopje-Ohrid',                    dt1],
    [1, '2026-02-03', 82890,  83600,  'Ruta Skopje-Gevgelija',                dt1],
    [1, '2026-02-18', 83600,  84100,  'Lokalna dostava',                      dt1],
    [1, '2026-03-07', 84100,  85200,  'Ruta Skopje-Tetovo-Gostivar',          dt1],
    [1, '2026-03-20', 85200,  87340,  'Megunarodna ruta Skopje-Beograd',      dt1],
    [2, '2026-01-08', 120000, 120780, 'Ruta Skopje-Prilep',                   dt2],
    [2, '2026-01-22', 120780, 121540, 'Ruta Skopje-Stip',                     dt2],
    [2, '2026-02-10', 121540, 122800, 'Ruta Skopje-Veles-Kavadarci',          dt2],
    [2, '2026-03-01', 122800, 124500, 'Ruta Skopje-Kumanovo-Vranje',          dt2],
    [3, '2026-02-05', 44000,  44450,  'Dostava na stoka vo Skopje',           dt3],
    [3, '2026-02-20', 44450,  44900,  'Ruta Skopje-Tetovo',                   dt3],
    [3, '2026-03-10', 44900,  45200,  'Dostava Aerodrom-Centar',              dt3],
    [5, '2026-01-10', 194000, 195200, 'Transport do Solun',                   dt4],
    [5, '2026-02-08', 195200, 196800, 'Ruta Skopje-Nis',                      dt4],
    [5, '2026-03-05', 196800, 198700, 'Transport Skopje-Zagreb',              dt4],
    [6, '2026-01-20', 60000,  61100,  'Lokalna dostava Skopje',               dt5],
    [6, '2026-02-15', 61100,  62400,  'Ruta Skopje-Kicevo',                   dt5],
    [6, '2026-03-12', 62400,  63800,  'Ruta Skopje-Strumica',                 dt5],
  ];
  ...
  console.log(`  ${mileage.length} mileage logs done`);
  ...
  console.log('Defects...');
  const def = [
    [1, 'Abenje na kocnici',                  'Se slusha shkripen.e pri kochenje, prednite plochki treba zamena',              'HIGH',   dt1],
    ...
    [3, 'Tekenje na maslo',                    'Zabele.ano maslo pod voziloto, malo tekenje od zaptivkata',                     'HIGH',   dt3],
    [4, 'Grebnati.na na predniot branik',      'Lesna grebnati.na pri manevriranje vo skladi.shteto',                           'LOW',    adm],
    [5, 'Vibracii pri vozenje',                'Pri brzina nad 80kmh vibracii na volanot, nebalansirani trkala',                'MEDIUM', dt4],
    ...
    [6, 'Greshka senzor pritisok gumi',        'TPMS indikatorot postojano sveti, verojtno neispraven senzor',                  'LOW',    dt5],
  ];
